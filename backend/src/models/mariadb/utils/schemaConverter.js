...
const validateFieldDefinition = (key, value) => {
  ...
  if (typeof value.type === 'string') {
    ...
    if (!sequelizeType) {
      throw new Error(`필드 "${key}"의 타입이 올바르지 않습니다: ${value.type}`);
    }
  ...
  if (!Object.values(DataTypes).includes(value.type)) {
    throw new Error(`필드 "${key}"의 정의가 올바르지 않습니다. DataTypes가 필요합니다. type: ${value.type}`);
  }
};
...
const applyFieldAttributes = (key, value, sequelizeField) => {
  ...
  if (value.trim && (value.type === DataTypes.STRING || value.type === DataTypes.TEXT)) {
