...
class TeamService {
  ...
  async validateInvitationRecord(teamId, inviteeId) {
    const invitation = await this.teamRepository.findInvitationRecord(
      teamId,
      inviteeId
    );
    if (!invitation) {
  ...
  async inviteMember(teamId, inviteeId, inviterId, invitationMessage = "") {
    ...
    await this.teamRepository.inviteMember(
      teamId,
      inviteeId,
      inviterId,
      invitationMessage
  ...
  async handleTeamMemberAction(teamId, userId, action) {
    switch (action) {
      case constants.TEAM_MEMBER_ACTIONS.ACCEPT:
        ...
        await this.teamRepository.acceptInvitation(
          teamId,
          userId,
          invitation.role
        );
